		...
		foreach ($pages as $pg) {
			...
			if ($i != 1) {
				$toc .= html_writer::link(
					new moodle_url('move.php', array('id' => $cm->id, 'pageid' => $pg->id, 'up' => '1', 'sesskey' => $USER->sesskey)),
					$OUTPUT->pix_icon('t/up', get_string('up')),
					array('title' => get_string('up'))
				);
			}
			if ($i != count($pages)) {
				$toc .= html_writer::link(
					new moodle_url('move.php', array('id' => $cm->id, 'pageid' => $pg->id, 'up' => '0', 'sesskey' => $USER->sesskey)),
					$OUTPUT->pix_icon('t/down', get_string('down')),
					array('title' => get_string('down'))
				);
			}
			$toc .= html_writer::link(
				new moodle_url('edit.php', array('cmid' => $pg->cmid, 'id' => $pg->id, 'sesskey' => $USER->sesskey)),
				$OUTPUT->pix_icon('t/edit', get_string('edit')),
				array('title' => get_string('edit'))
			);
			$toc .= html_writer::link(
				new moodle_url('delete.php', array('id' => $pg->cmid, 'pageid' => $pg->id, 'sesskey' => $USER->sesskey)),
			...
			if ($pg->hidden) {
				$toc .= html_writer::link(
					new moodle_url('show.php', array('id' => $pg->cmid, 'pageid' => $pg->id, 'sesskey' => $USER->sesskey)),
			...
			$toc .= html_writer::link(
				new moodle_url('edit.php', array('cmid' => $pg->cmid, 'pagenum' => $pg->pagenum, 'sesskey' => $USER->sesskey)),
				$OUTPUT->pix_icon('add', get_string('addafter', 'mod_icontent'), 'mod_icontent'),
				array('title' => get_string('addafter', 'mod_icontent'))
			);
...
function icontent_simple_paging_button_bar($pages, $cmid, $startwithpage = 1, $attrid = 'fgroup_id_buttonar')
{
	...
	return html_writer::div($controlbuttons, "simple-paging-buttonbar icontent-buttonbar mt-2", array('id' => $attrid, 'style' => 'display: flex; justify-content:flex-end;'));
}
...
function icontent_make_attempt_summary_by_page($pageid, $cmid)
{
	...
	if ($allownewattempts) {
		...
		$iconrepeatattempt = html_writer::link(
			new moodle_url(
				'deleteattempt.php',
				array('id' => $cmid, 'pageid' => $pageid, 'sesskey' => sesskey())
			),
...
function icontent_make_toolbar($page, $icontent)
{
	...
	$toolbar = html_writer::tag('div', $highcontrast . $update . $new, array('class' => 'toolbarpage '));
	// Return toolbar
	return $toolbar;
}
/**
...
function icontent_get_fullpageicontent($pagenum, $icontent, $context)
{
	...
	$objpage->fullpageicontent = html_writer::tag('div', $toolbarpage . $title . $objpage->pageicontent . $npage . $qtsareas . $notesarea . $script, array('class' => 'fulltextpage', 'data-pagenum' => $objpage->pagenum, 'style' => icontent_get_page_style($icontent, $objpage, $context)));
	// Set page preview, log event and return page
	icontent_add_pagedisplayed($objpage->id, $objpage->cmid);
	\mod_icontent\event\page_viewed::create_from_page($icontent, $context, $objpage)->trigger();
