...
function extractTitleContent($string, $start, $end)
{
	$string = " " . $string;
	...
	$title = substr($string, $ini, $len);
	$lenToRemove = strlen($title) + 15;

	return array(
		$title,
		substr_replace($string, "", $startToRemove, $lenToRemove)
	);
}
...
if ($mform->is_cancelled()) {
	...
	if ($data->id) {
		...
		if (strpos($data->pageicontent_editor["text"], '[section]')) {

			$textContentArr = explode('[section]', $data->pageicontent_editor["text"]);

			for ($i = 0; $i < count($textContentArr); $i++) {
				// set page number
				$data->pagenum = (int)($i + 1);

				$rawContent = $textContentArr[$i];

				if (strpos($rawContent, "[title]") && strpos($rawContent, "[/title]")) {
					$result = extractTitleContent($rawContent, "[title]", "[/title]");

					// set title
					$data->title = $result[0];
					// set content
					$data->pageicontent_editor["text"] = $result[1];
				} else {
					$data->pageicontent_editor["text"] = $rawContent;
				}
